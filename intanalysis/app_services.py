...
class ChatHistoryManager:
    ...
    def save_chat(
        ...
        with self.db.connection() as conn:
            ...
            cursor.execute(
                """
                INSERT INTO chats (
                    user_id, query, explanation, markdown_response, stories_count,
                    stories_json, matched_entities_json, timing_json, created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def get_recent_chats(self, user_id: int, limit: int = 50) -> list[dict]:
        ...
        with self.db.connection() as conn:
            ...
            cursor.execute(
                """
                SELECT id, query, explanation, markdown_response, stories_count,
                       stories_json, matched_entities_json, timing_json, created_at
                FROM chats
                WHERE user_id = ?
